import { PayoffInputDebt } from './payoff-simulation.service';
...
describe('PayoffSimulationService', () => {
  const service = new PayoffSimulationService(new DebtPriorityService());

  const debt = (partial: Partial<PayoffInputDebt> & Pick<PayoffInputDebt, 'id' | 'creditor' | 'currentBalance' | 'annualRate' | 'installmentAmount'>): PayoffInputDebt => ({
  ...
  it('quita uma única dívida no prazo esperado', () => {
    // R$ 12.000 a 0% de juros, R$ 1.000/mês → 12 meses
  ...
  it('AVALANCHE minimiza juros totais em comparação à BOLA DE NEVE', () => {
    ...
    expect(avalanche.order[0].debtId).toBe('high'); // avalanche ataca o juro alto primeiro
    expect(snowball.order[0].debtId).toBe('low'); // bola de neve ataca o saldo menor primeiro
  });
  ...
  it('lança erro sem dívidas ativas', () => {
    expect(() =>
      service.simulate({ strategy: 'AVALANCHE', monthlyBudget: 500, debts: [] }),
    ).toThrow(/dívidas ativas/);
  });
