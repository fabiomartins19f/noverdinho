...
/**
 * Testes de integração (requerem PostgreSQL e Redis em execução):
 *   npm run db:up && npm run prisma:deploy && npm run test:e2e
 */
describe('Auth + Isolamento entre usuários (e2e)', () => {
  ...
  it('registra, autentica e usa o refresh token (rotação)', async () => {
    ...
    const replayed = await request(app.getHttpServer()).post('/api/auth/refresh').send({ refreshToken });
  ...
  it('um usuário nunca acessa dados de outro usuário', async () => {
    ...
    const a = await request(app.getHttpServer()).post('/api/auth/register').send({
      name: 'Usuário A', email: emailA, password: 'senha123',
    });
    const b = await request(app.getHttpServer()).post('/api/auth/register').send({
    ...
    // A lista apenas as próprias contas
  ...
  it('transações são idempotentes com Idempotency-Key', async () => {
    ...
    // Saldo atualizado apenas uma vez
    const me = await request(app.getHttpServer())
